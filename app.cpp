...
future<> app::main_async(unsigned int core) {
    ...
    auto ss = listen(seastar::make_ipv4_address({1234}), lo);
    if (closing) {
...
future<listener_ptr> app::add_listener(seastar::server_socket &&ss, unsigned int core) {
...
app::app() {
    seabrute::config::register_options(add_options());
}
...
int app::run(int ac, char ** av) {
    return app_template::run(ac, av, [this] {
        ...
        seastar::engine().at_exit(std::bind(&app::close, this));
        logger.debug("Will start {} listeners", seastar::smp::count);
