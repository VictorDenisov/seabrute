...
handle_connection (int * counter, connected_socket s, socket_address a) {
    ...
    return do_with(std::move(s), std::move(input), std::move(output), [counter] (connected_socket &ss, input_stream<char> &input, output_stream<char> &output) {
...
main_async(int * counter) {
    return do_with(listen(make_ipv4_address({1234})), [counter] (auto& listener) {
...
int main(int argc, char** argv) {
    ...
    app_template app;
    try {
        app.run(argc, argv, [&counter] {return main_async(&counter);});
